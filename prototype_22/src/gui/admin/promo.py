...
class MyPromoModel:
    ...
    def init_manage_data_entry(self, task, promo_name, promo_type, promo_percent, promo_description):
...
class MyPromoView(MyWidget):
    ...
    def set_promo_box(self):
        ...
        self.overview_table = MyTableWidget(object_name='overview_table')
        self.overview_prev_button = MyPushButton(text='Prev')
        self.overview_page_label = MyLabel(text=f"Page {self.m.page_number}/{self.m.total_page_number}")
        self.overview_next_button = MyPushButton(text='Next')
        self.overview_act_box = MyGroupBox()
        self.overview_act_layout = MyHBoxLayout()
        self.overview_act_layout.addWidget(self.overview_prev_button)
        self.overview_act_layout.addWidget(self.overview_page_label)
        self.overview_act_layout.addWidget(self.overview_next_button)
        self.overview_act_box.setLayout(self.overview_act_layout)
        self.overview_box = MyGroupBox()
        self.overview_layout = MyVBoxLayout()
        self.overview_layout.addWidget(self.overview_table)
        self.overview_layout.addWidget(self.overview_act_box,0,Qt.AlignmentFlag.AlignCenter)
        self.overview_box.setLayout(self.overview_layout)
        
        self.promo_sort_tab = MyTabWidget()
        self.promo_sort_tab.addTab(self.overview_box, 'Overview')
    ...
    def set_overview_table_act_box(self):
        ...
        self.overview_act_box = MyGroupBox()
        self.overview_act_layout = MyHBoxLayout()
        self.overview_act_layout.addWidget(self.edit_data_button)
        self.overview_act_layout.addWidget(self.view_data_button)
        self.overview_act_layout.addWidget(self.delete_data_button)
        self.overview_act_box.setLayout(self.overview_act_layout)
...
class MyPromoController:
    ...
    def set_promo_box_conn(self):
        ...
        self.v.overview_prev_button.clicked.connect(self.on_overview_prev_button_clicked)
        self.v.overview_next_button.clicked.connect(self.on_overview_next_button_clicked)
        pass
    ...
    def populate_overview_table(self, text='', page_number=1): # IDEA: src
        self.v.overview_prev_button.setEnabled(page_number > 1)
        self.v.overview_next_button.setEnabled(page_number < self.m.total_page_number)
        self.v.overview_page_label.setText(f"Page {page_number}/{self.m.total_page_number}")

        promo_data = schema.select_data_as_display(text=text, page_number=page_number)

        self.v.overview_table.setRowCount(len(promo_data))
        ...
        for i, data in enumerate(promo_data):
            ...
            self.v.overview_table.setCellWidget(i, 0, self.v.overview_act_box)
            self.v.overview_table.setItem(i, 1, promo_name)
            self.v.overview_table.setItem(i, 2, promo_type)
            self.v.overview_table.setItem(i, 3, promo_percent)
            self.v.overview_table.setItem(i, 4, promo_desc)
            self.v.overview_table.setItem(i, 5, datetime_created)
    ...
    def on_overview_prev_button_clicked(self):
        if self.m.page_number > 1: 
            ...
            self.v.overview_page_label.setText(f"Page {self.m.page_number}/{self.m.total_page_number}")
    ...
    def on_save_data_button_clicked(self, task):
        ...
        promo_description = self.v.promo_desc_field.toPlainText()

        self.m.init_manage_data_entry(task, promo_name, promo_type, promo_percent, promo_description)
