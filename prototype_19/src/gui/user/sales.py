...
class MySalesView(MyWidget):
    ...
    def populate_prod_list_table(self, text_filter='', txn_type='Retail', curr_page=1):
        ...
        for row_index, row_value in enumerate(self.model.a_prod_list):
            ...
            table_atc_button = MyPushButton(object_name='table_atc_button', text='Add')
            table_view_button = MyPushButton(object_name='table_view_button', text='View')
            table_act_panel_layout.addWidget(table_atc_button)
            table_act_panel_layout.addWidget(table_view_button)
            table_act_panel.setLayout(table_act_panel_layout)

            table_atc_button.clicked.connect(lambda _, row_value=row_value: self.populate_via_atc(row_value=row_value))
    ...
    def show_pymt_panel(self):
        ...
        self.show_numpad_button = MyPushButton(text='Show')
        self.show_numpad_button.clicked.connect(self.show_after_pymnt_panel)
...
class MySalesController:
    ...
    def set_connections(self):
        ...
        self.model.order_pay_button[i].clicked.connect(self.on_order_pay_button_clicked)
        

        # self.view.add_order_button.clicked.connect(self.view.populate_via_barcode)
        # self.view.table_atc_button.clicked.connect(self.view.populate_via_atc)
        # self.view.add_order_button.clicked.connect(self.view.populate_via_clear)
        # self.view.add_order_button.clicked.connect(self.view.populate_via_drop_all)
        # self.view.add_order_button.clicked.connect(self.view.populate_via_add)
        # self.view.add_order_button.clicked.connect(self.view.populate_via_drop)
        # self.view.add_order_button.clicked.connect(self.view.populate_via_edit)
        pass
    ...
    def on_order_lock_button_clicked(self, action):
        ...
        if action == 'unlocked':
            ...
            self.model.order_a_clr_list_button[i].setDisabled(True)
            ...
            self.model.order_a_clr_list_button[i].setDisabled(False)
    ...
    def on_order_pay_button_clicked(self):
        self.view.show_pymt_panel()
        pass

    def on_show_numpad_button_clicked(self):
