...
from database.user.sales_unfinished import *
from widget.user.sales_unfinished import *
...
class SalesWindow(MyWidget):
    ...
    def default_init(self):
        ...
        self.current_user_data = 'Sample Cashier Name' # set to None for the mean time
            ...
            if float(self.amt_tendered_field.text()) >= float(self.total_value[i]):
                ...
                if confirm is QMessageBox.StandardButton.Yes:
                    ...
                    self.receipt_generator = ReceiptGenerator(
                        ...
                        customer_summary_data=self.customer_summary_data,
                        current_user=self.current_user_data # set to None for the mean time
                    )
    ...
    def on_sales_mgt_pay_button_clicked(self):
        ...
        if self.cart_list_table[i].rowCount() > 0:
            ...
            # customer_id = self.sales_schema.get_customer_id(customer=customer)
            customer_id = '1' # temporary
            print('customer_id:', customer_id)

            self.customer_summary_data.append([customer_id, customer])
    ...
    def populate_cart_tab(self): # cart process a
        ...
        print('self.customer_name_field.currentText():', self.customer_name_field.currentText())
        cart_tab_name = self.customer_name_field.currentText()
        ...
        print('cart_tab_name:', cart_tab_name)
    ...
    def show_extra_info_panel(self):
        ...
        self.current_user = MyLabel(object_name='current_user', text=f'Current user: {self.current_user_data}')
